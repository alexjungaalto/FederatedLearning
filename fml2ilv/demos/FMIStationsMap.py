...
sample_size = int(len(coordinates) * 0.15)
...
# Subset of the data corresponding to the cluster means 
...
### store the subset of FMI stations along with their lat/lon in csv 
...
#plt.plot(lons, lats, marker='o', linestyle='-', markersize=5, label='Polygon Border')
    ...
    # Annotate the stations with their names
for i, name in enumerate(sample_stationname):
    plt.text(sample_longitudes[i], sample_latitudes[i]+0.2, str(i), fontsize=15, ha='right')
        
    # Remove axes
plt.axis('off')
plt.savefig("FLNetFMI.png", dpi=100)
plt.show()
...
# -> 'degC'
data_dict= obs.data['Inari Seitalaassa']

# Assuming the dictionary is stored in a variable named `data_dict`
df = pd.DataFrame({
