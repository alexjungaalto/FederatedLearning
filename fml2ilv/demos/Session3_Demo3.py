...
# The graph topology defines who can exchange pseudo-labels with whom.
# Here: two nodes connected by one edge → each device has one neighbour.
...
for fl_iter in range(1, N_ITER + 1):
    ...
    for dev in devices:
        # ŷ^(i) = model_i(X̃^(i)) — predict on own synthetic features
        dev["y_pseudo"] = dev["model"].predict(dev["X_synth"])

    # Exchange: each device receives (X̃^(j), ŷ^(j)) from neighbours j ∈ N(i)
    for n in G.nodes:
    ...
    for dev in devices:
        ...
        # Augment: stack local (X^(i), y^(i)) with received (X̃^(j), ŷ^(j))
        X_aug = np.vstack([dev["X"],      dev["X_recv"]])
        y_aug = np.concatenate([dev["y"], dev["y_recv"]])

        model_new = dev["model_cls"]()     # fresh model of the same type
        model_new.fit(X_aug, y_aug)        # re-train on augmented dataset
        ...
        dev["model_prev"] = dev["model"]   # keep old model for comparison overlay
        dev["model"]      = model_new      # updated model used in next iteration
