...
empirical graph modelling, variation minimisation approach, and the FL algorithms used.
Do not include detailed numerical results. Instead, focus on problem, method, and evaluation setup.
	\end{hint}
\end{abstract}

\begin{keywords}
Federated learning, networks, personalised machine learning, trustworthy artificial intelligence
\end{keywords}
...
\begin{itemize}
	...
	\item Your choice of FL algorithm (i.e., optimisation method for solving
...
Instead, the report itself must provide sufficient detail on the data source
(e.g., which FMI stations, time range, variables), feature/label construction,
model specification, loss functions, and algorithm hyper-parameters so that the
numerical experiments can be \textbf{reproduced using a large language model (LLM)}
such as Mistral Le Chat or Aleph Alpha. In particular, a reader should be able to paste the
relevant sections of your report into an LLM and obtain code that reproduces your
results.
\end{hint}
