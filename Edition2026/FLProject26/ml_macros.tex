...
\newcommand{\estlocalparamsiter}[2]{\widehat{\weights}^{(#1,#2)}}
...
\newcommand{\learntlocalhypothesisiter}[2]{\widehat{\hypothesis}^{(#1,#2)}}
\newcommand{\discrepancy}[2]{d^{(#1,#2)}}  % discrepancy between local models of nodes #1 and #2
\newcommand{\sphere}[1]{\mathbb{S}^{(#1)}}  % the unit sphere in #1 dimensions
\newcommand{\firstnatural}[1]{[#1]}    % the set of first #1 natural numbers {1,2,..., #1} 

\newcommand{\metric}[2]{d(#1,#2)}  % metric between #1 and #2
\newcommand{\innerprod}[2]{\langle #1, #2 \rangle}  % inner product between #1 and #2
\newcommand{\hilbertspace}{\mathcal{H}}  % Hilbert space
\newcommand{\banachspace}{\mathcal{B}}  % Banach space
...
\newcommand{\cols}[1]{{\rm cols}\left(#1 \right)}  % the set of vectors constituted by the columns of matrix 
\newcommand{\linspan}[1]{{\rm span}\left(#1 \right)}  % the span of the columns of matrix
\newcommand{\nullspace}[1]{{\rm null}\left(#1 \right)}  % the nullspace of matrix
\def \vepsilon{{\bm{\epsilon}}}
\newcommand{\indicatorfunc}[1]{\mathbb{I}_{#1}}

% Statistics and Probability Theory 
\newcommand{\simplex}[1]{\Delta^{#1}}
\newcommand{\meanvecgeneric}{{\bm \mu}} 
\newcommand{\samplemeanvecgeneric}{\widehat{\bm \mu}} 
\newcommand{\covmtxgeneric}{\mC} 
\newcommand{\indexset}{\mathcal{I}}
\newcommand{\samplecovmtxgeneric}{\widehat{\mC}} 
\newcommand{\errprob}{p_{\rm err}} 
\newcommand{\probmeasure}{\mathbb{P}} 
\newcommand{\prob}[1]{\mathbb{P}\left(#1\right)} 
\newcommand{\cdf}[2]{F^{(#1)}\left(#2\right)} 
\newcommand{\probdist}{\mathbb{P}}
\newcommand{\entropy}[1]{H\left(#1\right)}   % entropy of random variable #1
\newcommand{\pdf}[2]{p^{(#1)}\left(#2\right)}   % probability density function
\newcommand{\pmf}[2]{p^{(#1)}\left(#2\right)}   % probability mass function
\newcommand{\mgf}[2]{M_{#1}\left(#2\right)}   % moment generating function
\newcommand{\mgfder}[3]{M_{#1}^{(#2)}\left(#3\right)}   % moment generating function derivative
\newcommand{\meanvec}[1]{{\bm \mu}^{(#1)}}  % mean vector of group #1
%\newcommand{\covmtx}[1]{\mathbf{C}^{(#1)}}  % covariance matrix of group #1
\newcommand{\covmtx}[1]{\mathbf{C}^{(#1)}}  % covariance matrix of group #1
\newcommand{\samplemeanvec}[1]{\widehat{{\bm \mu}}^{(#1)}}  % sample mean vector of group #1
\newcommand{\samplecovmtx}[1]{\widehat{\mathbf{C}}^{(#1)}}  % sample covariance matrix of group #1
\newcommand{\expect}{\mathbb{E} }    % expectation operator
\newcommand{\cov}[2]{{\rm cov}\left(#1,#2\right)}    % covariance of random variables #1 and #2
\newcommand{\covfunc}[3]{{\rm cov}^{(#1)}\left(#2,#3\right)}  % covariance function of random process #1 evaluated at #2 and #3
\newcommand{\sigmaalgebra}{\Sigma}    % sigma algebra
...
\newcommand{\nrfeatures}{\featuredim}   %  number of features used to characterize a data point 
\newcommand{\featurelenraw}{\featuredim'}
\newcommand{\featurelen}{\featuredim}     % length of a feature vector
...
\newcommand{\nrcluster}{k}   % number of clusters
\newcommand{\nrseeds}{s}  
\newcommand{\featureidx}{j}  % feature index 
\newcommand{\clustermean}[1]{{\bm \mu}^{(#1)}} 
\newcommand{\localclustercentroiditer}[3]{{\vw}^{(#1,#2,#3)}}   % #1 = node index (e.g., "i"), #2= cluster index (e.g., "c") , #3 = iteration index
\newcommand{\localclustercentroid}[2]{{\vw}^{(#1,#2)}}   % #1 = node index (e.g., "i"), #2= cluster index (e.g., "c") 
\newcommand{\clustercentroid}[1]{{\vw}^{(#1)}} % #1 = cluster index (e.g., "c")
\newcommand{\clustercentroiditer}[2]{{\vw}^{(#1,#2)}} % #1 = cluster index (e.g., "c"), #2 = iteration index
\newcommand{\clustercov}{{\bm \Sigma}}  % ????
\newcommand{\target}{y}             % target variable
\newcommand{\error}{E} 
\newcommand{\augidx}{b}
...
\newcommand{\trainset}{\dataset^{(\rm t)}}
\newcommand{\valset}{\dataset^{(\rm v)}}
...
\newcommand{\arm}{a}    % careful when using this macros file for beamer slides, as \action is already used by beamer
\newcommand{\actionset}{\actionspace}
\newcommand{\actionspace}{\mathcal{A}}
\newcommand{\obstacles}{\mathcal{B}}
\newcommand{\statevaluefunc}[1]{v_{#1}} % state value function of a policy 
\newcommand{\valuefunc}{v} % generic value function 
...
\newcommand{\itervar}{t}
\newcommand{\iteridx}{t}
\newcommand{\iteridxinner}{r}
\newcommand{\discountfac}{\gamma}
...
\newcommand{\maxdelay}{B}
\newcommand{\batchsize}{B}
\newcommand{\batch}{\mathcal{B}}
\newcommand{\foldsize}{B}
\newcommand{\nriter}{R}
\newcommand{\opttol}{\varepsilon^{(\rm tol)}}
\newcommand{\bdsize}[1]{\left| \partial #1\right|}
\newcommand{\bd}[1]{\partial #1}

\newcommand{\poisoningrate}{\eta}
\newcommand{\thresholdfct}{\tau}
\newcommand{\complexconjugate}[1]{\overline{#1}}

\newcommand{\openball}[2]{\mathcal{B}_{#2}(#1)}
\newcommand{\closedball}[2]{\overline{\mathcal{B}}_{#2}(#1)}

% Convex Analysis 

\newcommand{\proximityop}[2]{{\rm\bf prox}_{#1}(#2)}   % proximity operator of function #1 evaluated at #2

\newcommand{\fixedpointop}{\mathcal{F}} 
\newcommand{\contractfac}{\kappa}
\newcommand{\identityop}{\mathrm{Id}}

% Federated Learning 


\newcommand{\localtrainset}[1]{\dataset^{(#1,{\rm t})}}
\newcommand{\localvalset}[1]{\dataset^{(#1,{\rm v})}}
...
\newcommand{\avgnodedegree}{\overline{d}}
\newcommand{\nodeidx}{i}
\newcommand{\nrnodes}{n}
...
\newcommand{\bregmandiv}[3]{D_{#3}\big( #1,#2 \big)}
